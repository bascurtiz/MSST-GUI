"""models/bs_roformer/bs_roformer_unwa_large.py
------------------------------------------------
Unwa's "BS Roformer Instrumental Large v2" architecture
(pcunwa/BS-Roformer-Large-Inst on Hugging Face).

Identical to the plain BS-Roformer except — per the author's README — the
MaskEstimator additionally runs a 4-layer axial (time + frequency)
TransformerBlock before its per-band MLP heads, and the MaskEstimator ends
in its own RMSNorm. Those extra modules produce state-dict keys such as
``mask_estimators.0.layers...`` / ``mask_estimators.0.norm.gamma`` that the
plain classes don't have, so their strict ``load_state_dict`` fails with
"Unexpected key(s) in state_dict".

Configs for this variant carry a top-level ``unwa_inst_large_2: true``
marker; the engine (utils/settings.py) uses that to route here. The code is
vendored verbatim from the author's bs_roformer.py side-car so the weights
match exactly — only imports are adapted to this repo and a docstring added.
"""
...
class MaskEstimator(Module):
    ...
    def __init__(
        ...
        # unwa's addition: a 4-layer axial (time + frequency) TransformerBlock
        # ahead of the per-band MLP heads.
        self.layers = ModuleList([])
