"""models/bs_roformer/bs_roformer_fno.py
------------------------------------------------
pcunwa / unwa BS-Roformer fork with an FNO (Fourier Neural Operator) mask
estimator instead of the stock per-band MLP head. Checkpoints carry
``mask_estimators.*.to_freqs.*.*.fno_blocks.*`` keys and require
neuraloperator >= 2.0. Configs mark this variant with top-level ``fno: true``.
"""
